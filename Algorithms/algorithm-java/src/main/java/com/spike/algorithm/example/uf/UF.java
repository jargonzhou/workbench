...
import com.spike.algorithm.support.io.StdIn;
import com.spike.algorithm.support.io.StdOut;
import com.spike.algorithm.support.trace.*;
...
public class UF implements IUF, Traceable {
    ...
    public UF(int n) {
        ...
        for (int i = 0; i < n; i++) {
            parent[i] = i;
            trace.add(Lists.newArrayList(
                    new TraceAction(TraceStep.he, new TraceLink(i, i)),
                    TraceAction.ns));
            rank[i] = 0;
        }
    }
    ...
    public int find(int p) {
        ...
        while (p != parent[p]) {
            int oldP = p;
            int oldPP = parent[p];
            int oldPPP = parent[parent[p]];
            trace.add(Lists.newArrayList(
                    new TraceAction(TraceStep.he, new TraceLink(oldPP, oldPPP)),
                    new TraceAction(TraceStep.re, new TraceLink(oldP, oldPP)),
                    new TraceAction(TraceStep.he, new TraceLink(oldP, oldPPP)),
                    TraceAction.ns));
    ...
    public void union(int p, int q) {
        ...
        if (rank[rootP] < rank[rootQ]) {
            int old = parent[rootP];
            parent[rootP] = rootQ;
            trace.add(Lists.newArrayList(
                    new TraceAction(TraceStep.re, new TraceLink(rootP, old)),
                    new TraceAction(TraceStep.he, new TraceLink(rootP, rootQ)),
                    TraceAction.ns));
        }
        else if (rank[rootP] > rank[rootQ]) {
            int old = parent[rootQ];
            parent[rootQ] = rootP;
            trace.add(Lists.newArrayList(
                    new TraceAction(TraceStep.re, new TraceLink(rootQ, old)),
                    new TraceAction(TraceStep.he, new TraceLink(rootQ, rootP)),
                    TraceAction.ns));
        }
        else {
            int old = parent[rootQ];
